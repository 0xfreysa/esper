...
#[tracing::instrument(name = "run_server", skip(config))]
pub async fn run_server(config: &NotaryServerProperties) -> Result<(), NotaryServerError> {
    ...
    let provider_processor = Processor::new(
        config.provider.json_path.clone(),
        config.provider.schema_url.clone(),
    )
    .await
    .map_err(|e| eyre!("Failed to initialize provider: {}", e))?;
    ...
    let verifying_key = notary_signing_key.verifying_key();
...
mod test {
    ...
    #[test]
    fn test_generate_p256_signing_key() {
